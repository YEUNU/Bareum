<template>
    <div class="container">
      <nav class="navbar fixed-top bg-white">
        <div class="container-fluid">
          <a class="navbar-brand" href="/mypage">
            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="black" class="bi bi-arrow-left" viewBox="0 0 16 16">
              <path fill-rule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8z"/>
            </svg>
          </a>
        </div>
      </nav>
      <div class="card-container" style="margin-top: 5%;">
  <div class="card" style="width: 100%; display:block;">
    <img src="" class="card-img-top" alt="">
    <div class="card-body" style="padding: 20px;">
      <h5 style="text-align: left;  margin-top: -5%;">닉네임</h5>
      <p class="card-text" style="padding-top: 5%;">나이 / 성별</p>
      <a href="#" class="btn btn-primary">개인정보 수정</a>
      <a href="#" class="btn btn-primary">내 리뷰 관리</a>
      <a href="#" class="btn btn-primary">찜 목록</a>
    </div>
  </div>
</div>

<div class="card-container">
  <div class="card" style="width: 100%; display:block;">
    <img src="" class="card-img-top" alt="">
    <div class="card-body">
      <h5 style="text-align: left;  margin-top: -5%;">건강데이터</h5>
      <div class="row">
    <div class="col-sm-8" style="padding-top: 5%;">
      <div class="d-flex justify-content-between" style="align-items:center;">
        <p class="card-text" style="padding-top: 5%;">나의 건강검진 기록</p>
        <a href="#" class="btn btn-primary">수정</a>
      </div>
    </div>
    <div class="col-sm-8">
      <div class="d-flex justify-content-between" style="align-items:center;">
        <p class="card-text">건강 상태, 복용 약물</p>
        <a href="#" class="btn btn-primary">수정</a>
      </div>
    </div>
  </div>
    </div>
  </div>
</div>

<div class="card-container">
  <div class="card" style="width: 100%; display:block;">
    <img src="" class="card-img-top" alt="">
    <div class="card-body">
      <h5 style="text-align: left;  margin-top: -5%;">내 영양제</h5>
      <p class="card-text">등록 등록 등록</p>
      <a href="#" class="btn btn-primary">건강 고민 & 섭취 목적 (수정)</a>
    </div>
  </div>
</div>

<div class="card-container">
  <div class="card" style="width: 100%; display:block;">
    <div class="card-body">
            <h5 style="text-align: left;">주문 / 배송 내역</h5>
            <h5 style="text-align: left;  margin-top: 10%;">알람 설정</h5>
    </div>
  </div>
</div>

<div class="card-container">
  <div class="card" style="width: 100%; display:block;">
    <div class="card-body">
        <h5 style="text-align: left;  margin-top: -5%;">고객지원</h5>
        <div class="m-title" style="margin-top: 5%">
            <h5 style="text-align: left;">이용약관</h5>
            <h5 style="text-align: left;  margin-top: 10%;">개인정보 처리방침</h5>
            <h5 style="text-align: left;  margin-top: 10%;">로그아웃</h5>
            <h5 style="text-align: left;  margin-top: 10%;">회원탈퇴</h5>
            <a href="#" class="btn btn-primary" style="margin-top: 10%; margin-right:15vh">문의하기</a>
            <a href="#" class="btn btn-primary" style="margin-top: 10%;">영양제 등록 요청</a>
        </div>
    </div>
  </div>
</div>
    </div>
  </template>
  
  <script>
  export default {
    components: {}
  }
  </script>
  
  <style lang="css">
  .container {
    width: 100%; /* 화면 너비에 꽉 차도록 설정 */
    height: 100%; /* 화면 높이에 꽉 차도록 설정 */
    display: block;
    margin-bottom: 10%;
  }
  
  .card-container {
    margin-top: 2%;
  }

  .btn {
  margin-right: 0.6vw; /* 버튼들 사이의 간격을 조절 */
  margin-top: 5px;
}
  </style>